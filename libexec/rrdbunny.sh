...
if [ $# -lt 1 ] ; then
	echo "args: "
	echo  " graph output rrd_bunny.sh a.file.rrd "
...
for file_cursor in $infiles; do 
	...
	for ds_cursor in $data_sources; do 
		...
		lines=" LINE1:bunny$uniqueds-max#$color:'$legend_max' $lines " 
		colorburst $((uniqueds+1))
		lines=" LINE1:bunny$uniqueds-min#$color:'$legend_min' $lines " 
		#lines=" LINE1:bunny$uniqueds-min#$color:'nnies/femptofornight' $lines " 
